...
def profile_data_dir() -> Path:
    """The active profile's own data folder — settings/database live here
    (see config/profiles.py). The default profile's data stays exactly
    where app_data_dir() already points every existing (pre-multi-profile)
