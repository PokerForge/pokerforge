# License

> **DRAFT — not legal advice.** A one-paragraph proprietary notice, not a
> substitute for review — pair this with `TERMS_OF_USE.md` (the actual
> usage terms) and have both reviewed together before relying on either.

Copyright (c) 2026 PokerForge LTD (United Kingdom). All rights reserved.

This software and its source code are proprietary. No part of this
repository may be copied, modified, distributed, or used to create
derivative works without prior written permission from the copyright
holder, except as explicitly granted to a licensed end user under
`TERMS_OF_USE.md`.

This is closed-source commercial software — this file exists to make
that explicit, not to grant any open-source rights.
